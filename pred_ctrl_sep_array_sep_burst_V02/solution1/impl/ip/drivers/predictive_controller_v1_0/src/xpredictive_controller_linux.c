...
typedef struct {
    u32 addr;
    u32 size;
} XPredictive_controller_uio_map;
...
typedef struct {
    ...
    XPredictive_controller_uio_map maps[ MAX_UIO_MAPS ];
} XPredictive_controller_uio_info;

/***************** Variable Definitions **************************************/
static XPredictive_controller_uio_info uio_info;
...
static int uio_info_read_name(XPredictive_controller_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPredictive_controller_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPredictive_controller_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPredictive_controller_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPredictive_controller_Initialize(XPredictive_controller *InstancePtr, const char* InstanceName) {
	XPredictive_controller_uio_info *InfoPtr = &uio_info;
