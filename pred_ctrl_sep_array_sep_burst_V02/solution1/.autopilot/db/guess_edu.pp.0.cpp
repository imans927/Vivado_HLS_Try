...
float guess_edu (float U_KK_a[3*4],float V_Gen_a_cpy[9*4*4],float U_unc_kk[3*4])
{_ssdm_SpecArrayDimSize(U_KK_a, 12);_ssdm_SpecArrayDimSize(V_Gen_a_cpy, 144);_ssdm_SpecArrayDimSize(U_unc_kk, 12);



 float educated_rho=0.0;

 float u_educated[3*4]={};


 u_educated_cal_1:for (int i=3*4 -3; i<3*4; i++)
 {
  u_educated[i]=(U_KK_a[i]);


 };

 u_educated_cal_2:for (int i=0; i < 3*4 -3 ; i++)
 {

  u_educated[i]=(U_KK_a[i+3]);
 ...
 roh_educated_row:for (int row=0; row < 3*4 ; row++)
 {
  temp_value=0.0f;

  roh_educated_col:for (int col=0; col < 3*4 ; col++){

   if (col<=row){

   temp_value+=(V_Gen_a_cpy[row*3*4 +col])*u_educated[col];
