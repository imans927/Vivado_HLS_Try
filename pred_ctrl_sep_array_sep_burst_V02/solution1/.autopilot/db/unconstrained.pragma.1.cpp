...

{
 ...
 rHAt_Mul_xkk_row:for (int row=0 ; row < 2*4 ; row++){
      temp[row]=0;
      rHAt_Mul_xkk_col:for (int col=0; col < 4; col++){

       accu[col]=(R_Hat_a[row*4 +col])*(X_KK_a[col]);
       }
...
# 53 "unconstrained.cpp"
 Theta_kk_row:for (int row=0 ; row < 3*4 ; row++){
   theta_kk[row]=0;
  Theta_kk_col:for (int col=0 ; col < 2*4 ; col++){
   theta_kk[row]+=(Y_Hat_a[col*3*4 +row])*(temp[col]-(Y_Ref_KK_a[col]));




  }
# 74 "unconstrained.cpp"
  if (row<3){
   theta_kk[row]=theta_kk[row]- 6.8e-3*(U_KK_a[row]);
  }

 }
# 92 "unconstrained.cpp"
 unconstrained_row:for (int row=0 ; row < 3*4 ; row++){

  unconstrained[row]=0;
   unconstrained_col:for (int col=0 ; col < 3*4 ; col=col+4){



    acc_part:for (int col_s=0;col_s<4;col_s++){

     accu[col_s]+=-(V_Mul_H_Inv_a[row*3*4 +col+col_s])*theta_kk[col+col_s];
    }
   }
