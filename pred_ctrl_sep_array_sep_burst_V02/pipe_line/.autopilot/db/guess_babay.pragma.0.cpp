...
float guess_babay (float V_Gen_a[9*4*4],float H_Hat_Inv_a[9*4*4],float U_unc_kk[3*4],float theta_kk[3*4])
{_ssdm_SpecArrayDimSize(V_Gen_a, 144);_ssdm_SpecArrayDimSize(H_Hat_Inv_a, 144);_ssdm_SpecArrayDimSize(U_unc_kk, 12);_ssdm_SpecArrayDimSize(theta_kk, 12);
#pragma HLS PIPELINE
# 7 "guess_babay.cpp"
 ...
 U_babay_cal_row:for (int row=0; row < 3*4 ; row++)
 {
  U_babay[row]=0;
  temp=0;

  U_babay_cal_col:for (int col=0; col < 3*4 ; col++){

   U_babay[row]+= -1 * (H_Hat_Inv_a[row*3*4 +col])*theta_kk[col];
 ...
 roh_babay_row:for (int row=0; row < 3*4 ; row++)
 {
  temp_value=0.0f;

  roh_babay_col:for (int col=0; col <= 3*4 ; col++){

   if (col<=row){
    temp_value+=(V_Gen_a[row*3*4 +col])*U_babay[col];
   }
