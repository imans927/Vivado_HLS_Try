...
void sph_dec (float V_Gen_a[9*4*4],float roh, float U_unc[3*4], float U_opt[3*4]){_ssdm_SpecArrayDimSize(V_Gen_a, 144);_ssdm_SpecArrayDimSize(U_unc, 12);_ssdm_SpecArrayDimSize(U_opt, 12);
 ...
 int level = 0;
 float dist_array[4*3] = {0.0f};
_ssdm_SpecArrayPartition( dist_array, 1, "COMPLETE", 0, "");
# 14 "sph_dec.cpp"
 ...
 int switch_point[4*3];
_ssdm_SpecArrayPartition( switch_point, 1, "COMPLETE", 0, "");
# 24 "sph_dec.cpp"

 int U[4*3];
_ssdm_SpecArrayPartition( U, 1, "COMPLETE", 0, "");
# 25 "sph_dec.cpp"
...
# 48 "sph_dec.cpp"
   ...
   dist_matmul:for (ii = 0; ii<12; ii++)
   {
    ...
    if (U[ii]== 1) {dist_matmul += (V_Gen_a[level*3*4 +ii]);}
    else if (U[ii]==-1){dist_matmul -= (V_Gen_a[level*3*4 +ii]);}
    }
   ...
   if (dist_temp <= roh + 1e-6) {
    if (level == 3*4 -1)
    {

     U_opt:for (jj = 0; jj < 3*4; ++jj)
     {
      U_opt[jj]= U[jj];
   ...
   Backtracking:for (kk = 3*4 -1; kk > 0; --kk)
   {
